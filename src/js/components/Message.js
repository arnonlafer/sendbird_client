...
class Message {
  ...
  _createAdminElement() {
    ...
    const msg = createDivEl({ className: styles['message-admin'], content: protectFromXSS('"' + this.message.message.content + '"') });
    const msgAuthor = createDivEl({ className: styles['message-admin'], content: protectFromXSS(this.message.message.author) });
    root.appendChild(msg);
    root.appendChild(msgAuthor);
    return root;
  }
